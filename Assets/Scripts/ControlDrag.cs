...
//给空间添加监听事件要实现的一些接口
public class ControlDrag : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler,
 IEndDragHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    Vector3 imgReduceScale = new Vector3(1.2f, 1.2f, 1); //设置图片缩放
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        if(mouseDrag.x>420f||mouseDrag.y<580f)
        {
            ...
            if (gameObject.name.Equals("Image-target"))
                ControlPage2.targetAdded = true;
            else if (gameObject.name.Equals("Image-atcg"))
                ControlPage2.ATCGAdded = true;
            else if (gameObject.name.Equals("Image-prime"))
