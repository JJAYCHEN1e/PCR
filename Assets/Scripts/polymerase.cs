...
public class polymerase : MonoBehaviour
{
    ...
    void OnMouseUp()
    {
        ...
        DOTween.To(() => timer, a => timer = a, 1, 3f).OnComplete(() => SpeechController.Speak("打开装有Taq酶的试管"));
        se.Append(hat.DOMove(GameObject.Find("酶开盖").transform.position,2f));
        DOTween.To(() => timer, a => timer = a, 1, 8f).OnComplete(() => SpeechController.Speak("采集Taq酶"));
        ...
        DOTween.To(() => timer, a => timer = a, 1, 21f).OnComplete(() => GameObject.Find("酶吸头").GetComponent<SpriteRenderer>().enabled = true);
