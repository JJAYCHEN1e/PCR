...
public class rawMaterial : MonoBehaviour
{
    ...
    void Update()
    {
        if(type == "2.5") {
            ...
            if(startCheck && s.position == GameObject.Find("2.5备份").transform.position){
                ...
                if(buffer.employed && primer.employed && polymerase.employed && template.employed) SpeechController.Speak("试剂全部采样结束，请点击PCR仪的盖子，将混合液放入PCR仪中");
                else SpeechController.Speak("原料采样结束，可以采样其他试剂");
            }
        }
        // else if(type == "10") {
    ...
    void OnMouseUp()
    {
        ...
        if(employed)
        {
            SpeechController.Speak("原料已采样完成，请勿重复采样");
            return;
        }
       ...
       if(!buffer.employed)
        ...
        DOTween.To(() => timer, a => timer = a, 1, 3f).OnComplete(() => SpeechController.Speak("打开装有原料的试管"));
        ...
        DOTween.To(() => timer, a => timer = a, 1, 8f).OnComplete(() => SpeechController.Speak("采集原料"));
        ...
        DOTween.To(() => timer, a => timer = a, 1, 21f).OnComplete(() => GameObject.Find("原料吸头").GetComponent<SpriteRenderer>().enabled = true);
