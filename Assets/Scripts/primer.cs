...
public class primer : MonoBehaviour
{
    ...
    void Update()
    {
        if(type == "2.5") {
            ...
            if(startCheck && s.position == GameObject.Find("2.5备份").transform.position){
                ...
                if(buffer.employed && polymerase.employed && rawMaterial.employed && template.employed) SpeechController.Speak("试剂全部采样结束，请点击PCR仪的盖子，将混合液放入PCR仪中");
                else SpeechController.Speak("引物采样结束，可以采样其他试剂");
            }
        }
        // else if(type == "10") {
    ...
    void OnMouseUp()
    {
        ...
        DOTween.To(() => timer, a => timer = a, 1, 3f).OnComplete(() => SpeechController.Speak("打开装有引物的试管"));
        se.Append(hat.DOMove(GameObject.Find("引物开盖").transform.position,2f));
        se.Append(s.DOMove(GameObject.Find("引物上方").transform.position ,2f));
        DOTween.To(() => timer, a => timer = a, 1, 8f).OnComplete(() => SpeechController.Speak("采集引物"));
        ...
        se.Append(s.DOMove(GameObject.Find("引物采样").transform.position,2f));
        se.Append(s.DOMove(GameObject.Find("引物上方").transform.position ,2f));
        se.Append(s.DOLocalRotate(new Vector3(-20f, 0f, 0f), 1f, RotateMode.WorldAxisAdd));
        se.Append(s.DOMove(GameObject.Find("试管内").transform.position,2f));
        se.Append(hat.DOMove(GameObject.Find("引物备份盖").transform.position ,2f));
        DOTween.To(() => timer, a => timer = a, 1, 18f).OnComplete(() => SpeechController.Speak("废弃枪头"));
        DOTween.To(() => timer, a => timer = a, 1, 21f).OnComplete(() => GameObject.Find("引物吸头").GetComponent<SpriteRenderer>().enabled = true);
